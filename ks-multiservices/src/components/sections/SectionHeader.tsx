...
export default function SectionHeader({ eyebrow, title, intro, className }: SectionHeaderProps) {
  return (
    <div className={className}>
      ...
      <Reveal delay={0.05}>
        <h2 className="mt-6 max-w-4xl font-condensed text-[length:var(--fs-h2)] uppercase leading-[0.92] tracking-[0.01em] text-bone [text-wrap:balance]">
          {title}
        </h2>
      </Reveal>
      {intro ? (
        <Reveal delay={0.1}>
          <p className="mt-6 max-w-xl text-[length:var(--fs-lead)] leading-relaxed text-[color:var(--color-text-muted)]">
            {intro}
          </p>
        </Reveal>
      ) : null}
